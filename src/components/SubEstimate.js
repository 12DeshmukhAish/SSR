...
const SubEstimateForm = () => {
  ...
  const navigateToPdfPreview = async () => {
    if (!expandedSubwork || !items[expandedSubwork]) {
      toast.error("Expand a subwork with items to generate PDF");
      return;
    }
  
    const subworkItems = items[expandedSubwork];
  
    // Fetch measurements for all items in the subwork
    const subworkItemsWithMeasurements = await Promise.all(
      subworkItems.map(async (item) => {
        try {
    ...
    // Now send that full subwork item+measurement data to the PDF Preview page
    localStorage.setItem("subRecordCache", JSON.stringify(subworkItemsWithMeasurements));
