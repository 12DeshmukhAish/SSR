...
// const [itemSearchQuery, setItemSearchQuery] = useState('');
  ...
  const fetchCurrentRevision = async () => {
    ...
    if (workOrderId) {
      ...
      fetchSubworks(workOrderInfo.reviseId, workOrderId);
    }
...
const saveItemProperties = async (itemId, consumptionMaterials) => {
  try {
    ...
    // First try using the fromConsumption API
    try {
      const fromConsumptionResponse = await fetch(
        `${API_BASE_URL}/txn-item-properties/fromConsumption?detailsItemId=${consumptionMaterials[0].detailedItemId || consumptionMaterials[0].ssrItemId}&txtItemId=${itemId}`,
        {
          method: 'POST',
          headers: {
            "Authorization": `Bearer ${jwtToken}`,
...
const viewItemDetails = async (item) => {
  try {
    ...
    try {
      ...
      if (detailedItemId) {
        ...
        if (materialsResponse.ok) {
          ...
          // Add materials to the combined details
          if (Array.isArray(materialsData)) {
            consumptionMaterials = materialsData;
            combinedDetails.consumptionMaterials = materialsData;
          }
        }
      }
    ...
    // 4. Fetch existing transaction item properties
    let existingProperties = [];
    try {
      const propertiesResponse = await fetch(`${API_BASE_URL}/txn-item-properties/byTxnItem/${item.id}`, {
        headers: {
      ...
      if (propertiesResponse.ok) {
        ...
        console.log("Existing transaction item properties:", propertiesData);
        
        if (Array.isArray(propertiesData) && propertiesData.length > 0) {
          existingProperties = propertiesData;
          combinedDetails.itemProperties = propertiesData;
        }
      }
    } catch (propError) {
      console.warn("Error fetching transaction item properties:", propError);
    ...
    // 5. If no existing properties found but we have consumption materials, try the fromConsumption API first
    if (existingProperties.length === 0 && consumptionMaterials.length > 0) {
      try {
        const fromConsumptionResponse = await fetch(
          `${API_BASE_URL}/txn-item-properties/fromConsumption?detailsItemId=${item.itemNo}&txtItemId=${item.id}`,
          {
            headers: {
              "Authorization": `Bearer ${jwtToken}`,
              "Accept": "*/*"
        ...
        if (fromConsumptionResponse.ok) {
          const fromConsumptionData = await fromConsumptionResponse.json();
          console.log("Properties from consumption API:", fromConsumptionData);
          
          if (Array.isArray(fromConsumptionData) && fromConsumptionData.length > 0) {
            existingProperties = fromConsumptionData;
            combinedDetails.itemProperties = fromConsumptionData;
          } else {
            // If fromConsumption doesn't return data, save the properties
            await saveItemProperties(item.id, consumptionMaterials);
            
            // Refetch the properties after saving
            const refreshPropertiesResponse = await fetch(`${API_BASE_URL}/txn-item-properties/byTxnItem/${item.id}`, {
              headers: {
                "Authorization": `Bearer ${jwtToken}`,
                "Accept": "*/*"
            ...
            if (refreshPropertiesResponse.ok) {
              const refreshedData = await refreshPropertiesResponse.json();
              if (Array.isArray(refreshedData)) {
                combinedDetails.itemProperties = refreshedData;
              }
            }
          }
        }
      } catch (fromConsumptionError) {
        console.warn("Error using fromConsumption API:", fromConsumptionError);
        
        // Fall back to manual save if the fromConsumption API fails
        try {
          await saveItemProperties(item.id, consumptionMaterials);
          ...
          console.warn("Error auto-saving properties:", saveError);
        }
      }
    }
  ...
  // Function to show measurement table for item
  // const [visibleMeasurements, setVisibleMeasurements] = useState({});
...
const renderMeasurementTable = (item) => {
  ...
  return (
    <MeasurementTable 
      ...
      multifloor={true}
      itemsFromParent={previousItems} // Renamed prop to avoid conflict
    />
  );
};
...
<table className="min-w-full bg-white">
  <thead className="bg-gray-100">
    <tr>
      ...
      <th className="py-2 px-3 text-left">Floor/Lift</th>
      <th className="py-2 px-3 text-center"></th>
    </tr>
  </thead>
  <tbody>
    
    {/* Add Item Row - Only shown when adding a new item */}
    {currentSubworkId === subwork.id && !editingItemId && showAddItemModal && (
      <tr className="bg-blue-50">
        ...
        <td className="py-2 px-3">
          <div className="relative">
            <div className="custom-select-container">
              ...
              >
                <span>{itemForm.itemNo || "Select an Item"}</span>
        ...
        {/* <td className="py-2 px-3">
          <select
            name="floorLiftRise"
            value={itemForm.floorLiftRise || "Ground floor"}
            onChange={handleItemFormChange}
            className="w-full p-2 border border-gray-300 rounded"
          >
    ...
    {currentSubworkId === subwork.id && editingItemId && showAddItemModal && (
      <tr className="bg-yellow-50">
        <td className="py-2 px-3">Edit</td>
        <td className="py-2 px-3">
          <div className="relative">
            <div className="custom-select-container">
              <div 
              ...
              {isItemDropdownOpen && (
                <div className="absolute z-50 w-full bottom-full mb-1 bg-white border border-gray-300 rounded shadow-lg">
                  <div className="sticky top-0 p-2 border-b border-gray-200 bg-white z-10">
                    <input
                      type="text"
                      placeholder="Search items..."
      ...
      .map((item, index) => (
        <React.Fragment key={item.id}>
          ...
          <tr className={index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
            ...
            <td className="py-2 px-3">{item.itemNo}</td>
            <td className="py-2 px-3">{item.category}</td>
            <td className="py-2 px-3">{item.descriptionOfItem}</td>
            {/* <td className="py-2 px-3">{item.floorLiftRise || "Ground floor"}</td> */}
            <td className="py-2 px-3 text-center">
