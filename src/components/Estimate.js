...
if (chapterObj && chapterObj.chapterCategory.toLowerCase().includes("building work")) {
  ...
  setIsMultipleFloor(false); // Or retrieve this from storage if needed
}
...
useEffect(() => {
  ...
  if (isEditMode) {
    ...
    if (chapterObj && chapterObj.chapterCategory.toLowerCase().includes("building work")) {
      setShowMultipleFloorCheckbox(true);
      // ✅ FIXED: Get actual multifloor value from localStorage for edit mode
...
const addWork = async () => {
  ...
  // ✅ Removed fkSsrId as it's not in the API structure
};
...
/>
  ...
  const selectedChapterObj = uniqueChapters.find(
    ...
    if (label.includes("building work")) {
      setShowMultipleFloorCheckbox(true);
      setIsMultipleFloor(false); // ✅ FIXED: Default to unchecked (0)
